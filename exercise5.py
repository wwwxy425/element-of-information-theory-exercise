...
def neural_style_transfer(config):
    ...
    if config['style_img_name2'] is not None:
        style_img_path2 = os.path.join(config['style_images_dir'], config['style_img_name2'])
        out_dir_name += '_' + os.path.split(style_img_path2)[1].split('.')[0]
    ...
    if style_img2 is not None:
        ...
        target_style_representation2 = [utils.gram_matrix(x) for cnt, x in enumerate(style_img2_set_of_feature_maps) if cnt in style_feature_maps_indices_names[0]]
        
        w = config['style_blend_weight']
        blended_style_rep = []
        for g1, g2 in zip(target_style_representation, target_style_representation2):
            blended_style_rep.append(w * g1 + (1.0 - w) * g2)
        target_style_representation = blended_style_rep
...
if __name__ == "__main__":
    ...
    parser.add_argument("--content_img_name", type=str, help="content image name", default='sign.jpg')
    parser.add_argument("--style_img_name", type=str, help="style image name", default='monet.jpg')
    parser.add_argument("--style_img_name2", type=str, help="second style image name", default='vg.jpg')
    ...
    parser.add_argument("--style_blend_weight", type=float, help="weight for blending styles (for first style image)", default=0.5)#风格1权重
